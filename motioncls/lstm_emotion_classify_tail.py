...
【追加機能】
- 学習時だけ「尾部（後半）からのランダムクロップ」を混ぜるオンライン拡張：
  --tail_crop --tail_min_ratio 0.6 --train_crop_len 200 --tail_mix_prob 0.5
- ディレクトリ一括推論：ラベルごとの精度、混同行列、CSV保存（--save_csv）
...
使い方（学習）例:
    python lstm_emotion_classify_tail.py --mode train --data_root data_TW \
      ...
      --tail_crop --tail_min_ratio 0.6 --train_crop_len 200 --tail_mix_prob 0.5 \
      --ckpt_dir checkpoints --ckpt_name bilstm2_h256_tail06_w200_mix05.pt \
      --log_file bilstm2_h256_tail06_w200_mix05.csv

使い方（ディレクトリ一括推論）例:
    python lstm_emotion_classify_tail.py --mode infer \
      --ckpt checkpoints/bilstm2_h256_do03_ep50.pt \
      --input_dir data/train \
      --save_csv preds_train.csv
"""
...
class EmotionSequenceDataset(Dataset):
    ...
    追加（学習時のみ有効なオンライン拡張）:
    - tail_crop: 系列の後半（tail）からランダム窓を切り出して混在させる
    - tail_min_ratio: 尾部の開始位置（0.6 → 後ろ40%を優先）
    - train_crop_len: 切り出し長（フレーム数）。0以下で末尾まで
    - tail_mix_prob: そのサンプルで尾部窓を使う確率（残りは元の全長）
    """
    ...
    def __init__(
        ...
        train: bool = False,
        tail_crop: bool = False,
        tail_min_ratio: float = 0.6,
        train_crop_len: Optional[int] = 200,
        tail_mix_prob: float = 0.5,
    ):
        super().__init__()
        self.root_dir = root_dir
        self.train = train
        self.tail_crop = tail_crop
        self.tail_min_ratio = tail_min_ratio
        self.train_crop_len = train_crop_len
        self.tail_mix_prob = tail_mix_prob
    ...
    def __getitem__(self, idx):
        ...
        # ---- 学習時のみ：尾部クロップを確率的に適用 ----
        if self.train and self.tail_crop and random.random() < self.tail_mix_prob:
            T = motion.shape[0]
            start_min = int(T * self.tail_min_ratio)
            start_min = max(0, min(start_min, T - 1))
            if self.train_crop_len is None or self.train_crop_len <= 0 or self.train_crop_len >= T:
                start, end = start_min, T
            else:
                max_start = max(0, T - self.train_crop_len)
                start_low = min(start_min, max_start)
                start = random.randint(start_low, max_start)
                end = start + self.train_crop_len
            # 破綻防止：最低長を確保
            if end - start < 20:
                start, end = max(0, T - 20), T
...
class LSTMClassifierPacked(nn.Module):
    """
    263入力/6クラス。forward は生 logits を返す（CrossEntropyLoss 用）。
    """
...
def make_loaders(data_root: str, batch_size: int, num_workers: int,
                 ...
                 tail_crop: bool, tail_min_ratio: float,
                 train_crop_len: Optional[int], tail_mix_prob: float):
    ...
    if os.path.isdir(val_dir) and any(fn.endswith(".npy") for fn in os.listdir(val_dir)):
        train_ds = EmotionSequenceDataset(
            ...
            tail_crop=tail_crop,
            tail_min_ratio=tail_min_ratio,
            train_crop_len=(None if (train_crop_len is not None and train_crop_len <= 0) else train_crop_len),
        ...
        class _SubDS(Dataset):
            def __init__(self, items, train=False, tail_crop=False, tail_min_ratio=0.6,
                         train_crop_len: Optional[int] = 200, tail_mix_prob=0.5):
                self.items = items
                self.train = train
                self.tail_crop = tail_crop
                self.tail_min_ratio = tail_min_ratio
        ...
        train_ds = _SubDS(
            ...
            tail_crop=tail_crop, tail_min_ratio=tail_min_ratio,
            train_crop_len=(None if (train_crop_len is not None and train_crop_len <= 0) else train_crop_len),
            tail_mix_prob=tail_mix_prob
        )
...
def train_main(args):
    ...
    train_loader, val_loader = make_loaders(
        ...
        seed=args.seed,
        tail_crop=args.tail_crop,
        tail_min_ratio=args.tail_min_ratio,
        train_crop_len=args.train_crop_len,
        tail_mix_prob=args.tail_mix_prob
    )
...
def main():
    ...
    # 追加: 尾部クロップ（学習時のみ）
    parser.add_argument("--tail_crop", action="store_true",
                        help="学習時に系列の後半（tail）から切り出して学習する")
    parser.add_argument("--tail_min_ratio", type=float, default=0.6,
                        help="tail の開始位置（0.6なら後ろ40%を優先）")
    parser.add_argument("--train_crop_len", type=int, default=200,
                        help="学習時の切り出しフレーム数。0以下で末尾まで全て")
    parser.add_argument("--tail_mix_prob", type=float, default=0.5,
                        help="各サンプルでtail窓を使う確率（残りは元の全長を使用）")
