...
"""
LSTM で (T,263) の可変長系列を 7感情に分類する学習・評価・推論ワンパックスクリプト
- データ：.npy（辞書 or 配列）。辞書なら 'motion' (T,263) と 'label' (文字列) を推奨。
- ラベル：joy, anger, sadness, surprise, fear, disgust, contempt の7種類のみ学習対象。
- ラベルが無い場合はファイル名から推定（文字列一致）。
- 学習：train/ と val/ が無ければ train から自動分割（層化）も可。
- 推論：単発（--input_npy）またはディレクトリ一括（--input_dir）に対応。

【オンライン拡張（学習時のみ）】
- 中心ランダム割合クロップ（Tの中央付近を中心に、全長の一定割合 L=round(T*ratio) を抽出）
  例: --center_crop --crop_ratio 0.6 --center_low 0.3 --center_high 0.7 --center_mix_prob 0.5
  → 中心 c を [0.3T, 0.7T] から一様サンプルし、長さ L の窓を c を中心に切り出して学習に投入。
  ※ 末尾のT-pose汚染を避けつつ、系列の本体を学習させる目的。
...
使い方（ディレクトリ一括推論）例:
    python lstm_emotion_classify_basic7.py --mode infer \
      --ckpt checkpoints/bilstm2_h256_center06_mix05_basic7.pt \
      --input_dir data/train \
      --save_csv preds_train_basic7.csv
"""
...
# ========== 0. 定数 ==========
EMOTIONS = ["joy", "anger", "sadness", "surprise", "fear", "disgust", "contempt"]
...
# ====================== 2. データセット定義 ======================
def _center_random_crop(motion: np.ndarray,
                        ratio: float,
                        center_low: float,
                        center_high: float) -> np.ndarray:
    """
    中心 c を [center_low*T, center_high*T] から一様サンプルし、
    長さ L=round(T*ratio) の窓を c を中心に切り出す。
    端は自動クリップ。最小長は20フレームを確保。
    """
    T = motion.shape[0]
    if T < 20:
        return motion
    ratio = float(np.clip(ratio, 0.05, 1.0))
    center_low = float(np.clip(center_low, 0.0, 1.0))
    center_high = float(np.clip(center_high, 0.0, 1.0))
    if center_high < center_low:
        center_low, center_high = center_high, center_low

    L = int(round(T * ratio))
    L = max(20, min(L, T))  # 20 <= L <= T

    c_min = int(round(T * center_low))
    c_max = int(round(T * center_high))
    c_min = max(0, min(c_min, T - 1))
    c_max = max(0, min(c_max, T - 1))
    if c_max < c_min:
        c_min, c_max = c_max, c_min

    c = random.randint(c_min, c_max) if c_max > c_min else c_min
    start = c - L // 2
    end = start + L
...
class EmotionSequenceDataset(Dataset):
    ...
    学習時オンライン拡張（任意）:
    - center_crop: True のとき、確率的に中心ランダム割合クロップを適用
    """

    def __init__(
        self,
        root_dir: str,
        allow_missing_label_from_name: bool = True,
        train: bool = False,
        center_crop: bool = False,
        crop_ratio: float = 0.6,
        center_low: float = 0.3,
        center_high: float = 0.7,
        center_mix_prob: float = 0.5,
    ):
        super().__init__()
        ...
        self.center_mix_prob = center_mix_prob
    ...
    def __getitem__(self, idx):
        ...
        # ---- 学習時のみ：中心ランダム割合クロップを確率的に適用 ----
        if self.train and self.center_crop and random.random() < self.center_mix_prob:
            motion = _center_random_crop(
                motion, ratio=self.crop_ratio,
                center_low=self.center_low, center_high=self.center_high
            )
...
class LSTMClassifierPacked(nn.Module):
    """
    263入力/7クラス。forward は生 logits を返す（CrossEntropyLoss 用）。
    """

    def __init__(self, input_size=263, hidden_size=120, num_classes=7,
...
def make_loaders(data_root: str, batch_size: int, num_workers: int,
                 ...
                 center_crop: bool, crop_ratio: float,
                 center_low: float, center_high: float,
                 center_mix_prob: float):
    train_dir = os.path.join(data_root, "train")
    val_dir = os.path.join(data_root, "val")

    if os.path.isdir(val_dir) and any(fn.lower().endswith(".npy") for fn in os.listdir(val_dir)):
        train_ds = EmotionSequenceDataset(
            train_dir, train=True,
            center_crop=center_crop,
            crop_ratio=crop_ratio,
            center_low=center_low,
            center_high=center_high,
            center_mix_prob=center_mix_prob
        )
        val_ds = EmotionSequenceDataset(val_dir, train=False)
    else:
        # train の中だけに .npy を入れておき、自動で層化分割
        full_ds = EmotionSequenceDataset(train_dir, train=False)
        ...
        class _SubDS(Dataset):
            def __init__(self, items, train=False,
                         center_crop=False, crop_ratio=0.6,
                         center_low=0.3, center_high=0.7,
                         center_mix_prob=0.5):
                self.items = items
                self.train = train
                self.center_crop = center_crop
            ...
            def __getitem__(self, i):
                ...
                return x, motion.shape[0], y, path

        train_ds = _SubDS(
            train_items, train=True,
            center_crop=center_crop, crop_ratio=crop_ratio,
            center_low=center_low, center_high=center_high,
            center_mix_prob=center_mix_prob
        )
        val_ds = _SubDS(val_items, train=False)
...
def train_main(args):
    ...
    train_loader, val_loader = make_loaders(
        ...
        center_mix_prob=args.center_mix_prob
    )
...
def infer_one(args):
    ...
    motion, label_str = _load_motion_and_label(args.input_npy)
    ...
    if overall_total > 0:
        ...
        for emo in EMOTIONS:
            ...
            if t > 0:
                ...
                print(f"  {emo:9s}: {acc:.4f} ({correct_per[emo]}/{t})")
...
def main():
    ...
    # 追加: 中心ランダム割合クロップ（学習時のみ）
    parser.add_argument("--center_crop", action="store_true",
                        help="学習時に中心付近を中心とする割合ウィンドウで切り出して学習する")
    parser.add_argument("--crop_ratio", type=float, default=0.6,
                        help="切り出し長の割合（0.6 → 全長の60%）")
    parser.add_argument("--center_low", type=float, default=0.3,
                        help="中心 c の下限（c ∈ [center_low*T, center_high*T]）")
    parser.add_argument("--center_high", type=float, default=0.7,
                        help="中心 c の上限（c ∈ [center_low*T, center_high*T]）")
    parser.add_argument("--center_mix_prob", type=float, default=0.5,
                        help="各サンプルで中心割合クロップを使う確率（残りは元の全長を使用）")
