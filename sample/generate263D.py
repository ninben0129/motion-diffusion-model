...
No video generation / visualization is performed.
"""
...
def main():
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # 生成（注意：学習と整合するために max_frames 形状で生成）
        sample = sample_fn(
            model,
        ...
        # sample: [B, njoints(or 263), nfeats, T=max_frames] 相当
        # HumanML3Dベクトル表現のまま保持するため、ここからの座標再構成/rot2xyzは行わない。
        ...
        # 長さを取り出し
        lengths = model_kwargs['y']['lengths'].detach().cpu().numpy()  # shape [B]

        # ===== サンプルごとに個別保存 =====
        # 保存する際は length までで切り出して軽量化
        for sample_i in range(args.num_samples):
            length = int(lengths[sample_i])
            caption = text_list[sample_i] if sample_i < len(text_list) else ''

            # [njoints/feat, nfeats?, T]にあわせて切り出し（もとの配列の軸構成は維持）
            ...
            # 必要メタも同梱
            payload = {
                'motion': one_motion,               # HumanML3D vector representation（そのまま）
                'text': caption,
                'length': length,
                'num_samples_in_batch': int(args.num_samples),
                'repetition_index': int(rep_i),
            ...
            print(f'[Saved] {save_path} | length={length} | text="{caption}"')
